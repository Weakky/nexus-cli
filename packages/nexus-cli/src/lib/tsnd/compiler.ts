...
let tsHandler: ((m: any, filename: string) => any) | null = null;
let tmpDir = ".ts-node";
const extensions = [".ts", ".tsx"];
const empty = function() {};
const cwd = process.cwd();
const compilationInstanceStampt = Math.random()
  .toString()
  .slice(2);
...
export const compiler: Compiler = {
  ...
  getCompilationId: function() {
    return compilationInstanceStampt;
  },
  ...
  getCompilerReadyFilePath: function() {
    return path
      .join(os.tmpdir(), "ts-node-dev-ready-" + compilationInstanceStampt)
      .replace(/\\/g, "/");
  },
  getChildHookPath: function() {
    return path
      .join(
        os.tmpdir(),
        "ts-node-dev-hook-" + compilationInstanceStampt + ".js"
      )
      .replace(/\\/g, "/");
  },
